 ...
 */
class BrowserLogViewerController extends Controller
{
    /* -----------------------------------------------------------------
     |  Properties
     | -----------------------------------------------------------------
    ...
    /** @var string */
    protected $showRoute = 'log-viewer::nocs.logs.show';
    ...
    public function __construct(LogViewerContract $logViewer)
    {
        ...
        $this->logViewer->setPath(storage_path('logs/nocs'));
        $this->viewPrefix = 'nocs';
        $this->logViewer->setPattern('nocs-');
    }
    ...
    public function listLogs(Request $request)
    {
        ...
        $requests = config('log-viewer.colors.request');

        return $this->view('logs', compact('headers', 'rows', 'requests'));
    }
    ...
    public function show(Request $request, $date)
    {
        ...
        return $this->view('show', compact('level', 'log', 'query', 'levels', 'entries', 'requests'));
    }
        ...
        $entries = $log->entries($level)
            ->unless(empty($needles), function (LogEntryCollection $entries) use ($needles) {
                return $entries->filter(function (LogEntry $entry) use ($needles) {
                    return Str::containsAll(Str::lower(json_encode($entry->context)), $needles);
                });
            })
    ...
    protected function view($view, $data = [], $mergeData = [])
    {
        ...
        return view()->make("log-viewer::{$theme}.{$this->viewPrefix}.{$view}", $data, $mergeData);
    }
